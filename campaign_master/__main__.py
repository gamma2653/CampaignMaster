...
parser = argparse.ArgumentParser(
    description="Campaign Master - Manage your tabletop RPG campaigns"
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    if known_args.web:
        ...
        util.build()
        if input(
            "Would you like to run the web server now? (y/n): "
        ).strip().lower() not in ("y", "yes", "1"):
            sys.exit(0)
        if known_args.debug:
            web_app.run_dev(
                host=known_args.host, port=known_args.port, debug=known_args.debug
            )
        # else:
    ...
    if known_args.gui:
        ...
        from campaign_master.content.database import (
