...
vi.mock('../../features/shared/components/ctx', () => ({
    useAppForm: (opts: { defaultValues: unknown; onSubmit: (data: { value: unknown }) => void }) => {
        return {
            state: { values: opts.defaultValues },
            handleSubmit: mockHandleSubmit,
            AppForm: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
            SubscribeButton: ({ label, className }: { label: string; className?: string }) => (
...
function TestCampaignPlanEditForm() {
    ...
    const { data: campaign, isLoading, error } = mockUseCampaignPlanByID({
...
describe('CampaignPlanEdit Route', () => {
    ...
    describe('error state', () => {
        it('should display error message when fetch fails', () => {
            ...
            render(<TestCampaignPlanEditForm />);
            expect(screen.getByText('Error loading campaign: Network error')).toBeInTheDocument();
        });
    });
    ...
    describe('form submission', () => {
        it('should call handleSubmit when form is submitted', () => {
            ...
            const form = screen.getByRole('button', { name: 'Save Campaign' }).closest('form');
