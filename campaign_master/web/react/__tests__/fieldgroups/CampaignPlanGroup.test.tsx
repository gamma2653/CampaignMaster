...
vi.mock('@headlessui/react', () => ({
    ...
    DisclosureButton: ({ children, className, onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) => (
        <button className={className} onClick={onClick} data-testid="disclosure-button">
            {children}
        </button>
    ),
    DisclosurePanel: ({ children, className }: { children: React.ReactNode; className?: string }) => (
...
vi.mock('@heroicons/react/20/solid', () => ({
    ChevronDownIcon: ({ className }: { className?: string }) => (
        <span className={className} data-testid="chevron-icon">v</span>
    ),
}));
...
describe('CampaignPlanGroup', () => {
    ...
    describe('main structure', () => {
        it('should render Campaign Plan heading', () => {
            ...
            expect(screen.getByRole('heading', { name: 'Campaign Plan' })).toBeInTheDocument();
        });

        it('should have campaign-plan-group id', () => {
            render(<CampaignPlanGroup />);
            expect(document.getElementById('campaign-plan-group')).toBeInTheDocument();
        });
    });
    ...
    describe('disclosure sections', () => {
        ...
        it('should render Storyline Arcs section', () => {
            render(<CampaignPlanGroup />);
            expect(screen.getByText('Storyline Arcs (continuous)')).toBeInTheDocument();
        });
    ...
    describe('add buttons', () => {
        it('should render Add Story Point button', () => {
            render(<CampaignPlanGroup />);
            expect(screen.getByRole('button', { name: 'Add Story Point' })).toBeInTheDocument();
        });

        it('should render Add Storyline Arc button', () => {
            render(<CampaignPlanGroup />);
            expect(screen.getByRole('button', { name: 'Add Storyline Arc' })).toBeInTheDocument();
        });
    ...
    describe('entity arrays', () => {
        it('should render existing storypoints', () => {
            ...
            render(<CampaignPlanGroup />);
            expect(screen.getByTestId('point-group-storypoints[0]')).toBeInTheDocument();
            expect(screen.getByTestId('point-group-storypoints[1]')).toBeInTheDocument();
        });
        ...
        it('should render existing characters', () => {
            ...
            render(<CampaignPlanGroup />);
            expect(screen.getByTestId('character-group-characters[0]')).toBeInTheDocument();
        });
        ...
        it('should render existing locations', () => {
            ...
            render(<CampaignPlanGroup />);
            expect(screen.getByTestId('location-group-locations[0]')).toBeInTheDocument();
        });
        ...
        it('should render existing rules', () => {
            mockFieldValues.rules = [
                { obj_id: { prefix: PREFIXES.RULE, numeric: 1 }, description: 'Rule 1' },
            ];
            render(<CampaignPlanGroup />);
        ...
        it('should render existing objectives', () => {
            mockFieldValues.objectives = [
                { obj_id: { prefix: PREFIXES.OBJECTIVE, numeric: 1 }, description: 'Obj 1' },
            ];
            render(<CampaignPlanGroup />);
            expect(screen.getByTestId('objective-group-objectives[0]')).toBeInTheDocument();
        });
    ...
    describe('section headers', () => {
        it('should render Storyline section header', () => {
            render(<CampaignPlanGroup />);
            expect(screen.getByRole('heading', { name: 'Storyline (continuous)' })).toBeInTheDocument();
        });
