...
import { ArcGroup, defaultValues } from '../../features/shared/components/fieldgroups/ArcGroup';
...
describe('ArcGroup', () => {
    ...
    describe('segments array', () => {
        it('should render existing segments', () => {
            const segment1 = createMockSegment({ obj_id: { prefix: PREFIXES.SEGMENT, numeric: 1 } });
            const segment2 = createMockSegment({ obj_id: { prefix: PREFIXES.SEGMENT, numeric: 2 } });
            mockFieldValues.segments = [segment1, segment2];

            render(<ArcGroup points={[]} />);

            expect(screen.getByTestId('segment-group-segments[0]')).toBeInTheDocument();
            expect(screen.getByTestId('segment-group-segments[1]')).toBeInTheDocument();
        });

        it('should pass points prop to SegmentGroup children', () => {
            const points = [
                createMockPoint({ obj_id: { prefix: PREFIXES.POINT, numeric: 1 }, name: 'Start' }),
