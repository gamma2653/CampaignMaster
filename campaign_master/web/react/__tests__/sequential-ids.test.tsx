...
describe('Sequential ID Creation - Mutation Hooks', () => {
    ...
    describe('useCreatePoint', () => {
        it('should receive sequential IDs starting from 1', async () => {
            ...
            mockFetch
                .mockResolvedValueOnce({
                    ok: true,
                    json: () => Promise.resolve(createMockPoint({ obj_id: createMockPointID(1) })),
                })
                .mockResolvedValueOnce({
                    ok: true,
                    json: () => Promise.resolve(createMockPoint({ obj_id: createMockPointID(2) })),
                })
                .mockResolvedValueOnce({
                    ok: true,
                    json: () => Promise.resolve(createMockPoint({ obj_id: createMockPointID(3) })),
                });
            ...
            // Create first point
            const createData = { name: 'Test Point', description: 'Test', objective: null };
        ...
        it('should correctly parse Point ID from API response', async () => {
            ...
            result.current.mutate({ name: 'Test Point', description: '', objective: null });
            await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    describe('useCreateCharacter', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockCharacter({ obj_id: createMockCharacterID(1) })),
            });
    ...
    describe('useCreateRule', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockRule({ obj_id: createMockRuleID(1) })),
            });
            ...
            result.current.mutate({ description: 'Test Rule', effect: '', components: [] });
    ...
    describe('useCreateItem', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockItem({ obj_id: createMockItemID(1) })),
            });
            ...
            result.current.mutate({ name: 'Test Item', type_: 'weapon', description: '', properties: {} });
    ...
    describe('useCreateLocation', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockLocation({ obj_id: createMockLocationID(1) })),
            });
            ...
            result.current.mutate({ name: 'Test Location', description: '', neighboring_locations: [], coords: null });
    ...
    describe('useCreateObjective', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockObjective({ obj_id: createMockObjectiveID(1) })),
            });
            ...
            result.current.mutate({ description: 'Test Objective', components: [], prerequisites: [] });
    ...
    describe('useCreateArc', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockArc({ obj_id: createMockArcID(1) })),
            });
            ...
            result.current.mutate({ name: 'Test Arc', description: '', segments: [] });
    ...
    describe('useCreateSegment', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockSegment({ obj_id: createMockSegmentID(1) })),
            });
    ...
    describe('useCreateCampaignPlan', () => {
        it('should receive sequential IDs starting from 1', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve(createMockCampaign({ obj_id: createMockCampaignID(1) })),
            });
...
describe('Sequential ID Creation - Multiple Resources Simulation', () => {
    ...
    it('should maintain correct ID structure after create mutation', async () => {
        ...
        result.current.mutate({ name: 'New Point', description: '', objective: null });
...
describe('ID Sequence Independence', () => {
    ...
    it('should show different resource types can have same numeric ID', async () => {
        ...
        // Create Point
        pointResult.current.mutate({ name: 'Point', description: '', objective: null });
