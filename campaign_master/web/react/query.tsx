...
import type {
    ...
    RuleCreate, RuleUpdate, ObjectiveCreate, ObjectiveUpdate, PointCreate, PointUpdate,
    SegmentCreate, SegmentUpdate, ArcCreate, ArcUpdate, ItemCreate, ItemUpdate,
    CharacterCreate, CharacterUpdate, LocationCreate, LocationUpdate,
    CampaignPlanCreate, CampaignPlanUpdate, AgentConfigCreate, AgentConfigUpdate,
} from './schemas'
...
const [useAgentConfig, useAgentConfigByID] = generateShallowQueries<AgentConfigID, AgentConfig>([PREFIXES.AGENT_CONFIG])
...
// Mutation factories
const generateCreateMutation = <CreateType, RetType extends Object>(prefix: string) => {
    return () => {
...
const generateUpdateMutation = <UpdateType extends Object, RetType extends Object>(prefix: string) => {
    return () => {
        const queryClient = useQueryClient()
        return useMutation({
            mutationFn: async (data: UpdateType): Promise<RetType> => {
                const id = (data as { obj_id: AnyID }).obj_id
                const response = await fetch(`${BASE_API_URL}${prefix.toLowerCase()}/${id.numeric}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data),
                })
...
const useCreateObjective = generateCreateMutation<ObjectiveCreate, Objective>(PREFIXES.OBJECTIVE)
const useCreatePoint = generateCreateMutation<PointCreate, Point>(PREFIXES.POINT)
const useCreateSegment = generateCreateMutation<SegmentCreate, Segment>(PREFIXES.SEGMENT)
...
const useCreateCharacter = generateCreateMutation<CharacterCreate, Character>(PREFIXES.CHARACTER)
const useCreateLocation = generateCreateMutation<LocationCreate, Location>(PREFIXES.LOCATION)
const useCreateCampaignPlan = generateCreateMutation<CampaignPlanCreate, CampaignPlan>(PREFIXES.CAMPAIGN_PLAN)
const useCreateAgentConfig = generateCreateMutation<AgentConfigCreate, AgentConfig>(PREFIXES.AGENT_CONFIG)

// Update mutations
const useUpdateRule = generateUpdateMutation<RuleUpdate, Rule>(PREFIXES.RULE)
const useUpdateObjective = generateUpdateMutation<ObjectiveUpdate, Objective>(PREFIXES.OBJECTIVE)
const useUpdatePoint = generateUpdateMutation<PointUpdate, Point>(PREFIXES.POINT)
const useUpdateSegment = generateUpdateMutation<SegmentUpdate, Segment>(PREFIXES.SEGMENT)
...
const useUpdateCharacter = generateUpdateMutation<CharacterUpdate, Character>(PREFIXES.CHARACTER)
const useUpdateLocation = generateUpdateMutation<LocationUpdate, Location>(PREFIXES.LOCATION)
const useUpdateCampaignPlan = generateUpdateMutation<CampaignPlanUpdate, CampaignPlan>(PREFIXES.CAMPAIGN_PLAN)
const useUpdateAgentConfig = generateUpdateMutation<AgentConfigUpdate, AgentConfig>(PREFIXES.AGENT_CONFIG)
...
export {
    // Query hooks
    useRule, useRuleByID,
    useObjective, useObjectiveByID,
    usePoint, usePointByID,
    useSegment, useSegmentByID,
    useArc, useArcByID,
    useItem, useItemByID,
    useCharacter, useCharacterByID,
    useLocation, useLocationByID,
    useCampaignPlan, useCampaignPlanByID,
    useAgentConfig, useAgentConfigByID,
    usePointByCampaignAndID,
    // Create mutations
    useCreateRule, useCreateObjective, useCreatePoint, useCreateSegment, useCreateArc,
    useCreateItem, useCreateCharacter, useCreateLocation, useCreateCampaignPlan, useCreateAgentConfig,
    // Update mutations
    useUpdateRule, useUpdateObjective, useUpdatePoint, useUpdateSegment, useUpdateArc,
    useUpdateItem, useUpdateCharacter, useUpdateLocation, useUpdateCampaignPlan, useUpdateAgentConfig,
    // Delete mutations
    useDeleteRule, useDeleteObjective, useDeletePoint, useDeleteSegment, useDeleteArc,
    useDeleteItem, useDeleteCharacter, useDeleteLocation, useDeleteCampaignPlan, useDeleteAgentConfig,
}