...
import {
  CharacterGroup,
  defaultValues as characterDefaultValues,
} from './CharacterGroup';
import {
  LocationGroup,
  defaultValues as locationDefaultValues,
} from './LocationGroup';
...
import {
  ObjectiveGroup,
  defaultValues as objectiveDefaultValues,
} from './ObjectiveGroup';
          ...
          {({ open }) => (
            <>
              <DisclosureButton className="w-full p-2 text-left font-semibold text-lg cursor-pointer border border-blue-900 bg-blue-950">
                Campaign Plan Metadata
                <ChevronDownIcon
                  className={clsx('w-5 inline-block', open && 'rotate-180')}
                />
              </DisclosureButton>
              <DisclosurePanel className="flex flex-col gap-4 p-2">
                ...
                <div className="p-2 min-w-full">
                  <group.AppField name="version">
                    {(field) => (
                      <field.TextField label="Campaign Plan Version" />
                    )}
                  </group.AppField>
                </div>
                <div className="p-2 min-w-full">
                ...
                <group.AppField name="storyline" mode="array">
                  {(field) => (
                    <div className="p-2 min-w-full">
                      ...
                      <group.Subscribe
                        selector={(state) => state.values.storypoints}
                      >
                        {(storypoints) => (
                          <>
                  ...
                  {(field) => (
                    <div className="p-2 min-w-full">
                      ...
                      {group.state.values.items.map((_, index) => (
                        <div key={index} className="border p-2 mb-2">
                          {/* <h4>Item {index + 1}</h4> */}
                          <ItemGroup form={group} fields={`items[${index}]`} />
                        </div>
                      ))}
                      <button
                        type="button"
                        className="add-button"
                        onClick={() => field.pushValue(itemDefaultValues)}
                      >
                        Add Item
                      </button>
                    </div>
                  ...
                  {(field) => (
                    <div className="p-2 min-w-full">
                      ...
                      {group.state.values.rules.map((_, index) => (
                        <div key={index} className="border p-2 mb-2">
                          {/* <h4>Rule {index + 1}</h4> */}
                          <RuleGroup form={group} fields={`rules[${index}]`} />
                        </div>
                      ))}
                      <button
                        type="button"
                        className="add-button"
                        onClick={() => field.pushValue(ruleDefaultValues)}
                      >
                        Add Rule
                      </button>
                    </div>
                  ...
                  {(field) => (
                    <div className="p-2 min-w-full">
                      ...
                      <button
                        type="button"
                        className="add-button"
                        onClick={() => field.pushValue(objectiveDefaultValues)}
                      >
                        Add Objective
                      </button>
                    </div>
