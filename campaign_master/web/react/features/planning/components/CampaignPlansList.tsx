...
export function CampaignPlansList() {
    ...
    if (error) {
        return (
            <div className="flex items-center justify-center py-12">
                <div className="text-red-400">Error loading campaigns: {error.message}</div>
            </div>
        )
    }
        ...
        <div className="space-y-6">
            ...
            {!campaigns || campaigns.length === 0 ? (
                <div className="text-center py-12 bg-gray-800/30 rounded-lg border border-white/10">
                    <p className="text-gray-400">You don't have any campaigns yet. Click "New Campaign" in the navbar to create one.</p>
                </div>
            ) : (
